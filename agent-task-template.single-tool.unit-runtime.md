...
function Show-TestSummary($path) {
  ...
  $matches = Select-String -Path $path -Pattern 'ERR_ASSERTION|Expected values|actual:|expected:|AssertionError|not ok|ok |# pass|# fail|node --test' | Select-Object -First 26
...
$initialLog = Join-Path $env:TEMP 'mvp-unit-runtime-initial.log'
npm test *> $initialLog
...
$changed = $false
if ($initialCode -ne 0 -and (Select-String -Path $initialLog -Pattern 'ERR_ASSERTION|AssertionError|Expected values' -Quiet)) {
  $source = Get-Content index.js -Raw
  if ($source -match 'return value\.toLowerCase\(\);') {
    $source = $source -replace 'return value\.toLowerCase\(\);', 'return value.trim().toLowerCase();'
    Set-Content -LiteralPath index.js -Value $source -NoNewline
    $changed = $true
  } elseif ($source -match 'return values;') {
    $source = $source -replace 'return values;', 'return values.filter(value => value != null);'
    Set-Content -LiteralPath index.js -Value $source -NoNewline
    $changed = $true
  }
}

$finalLog = Join-Path $env:TEMP 'mvp-unit-runtime-final.log'
npm test *> $finalLog
