...
硬性约束：
- 只使用 1 次 shell/tool 调用完成检查、诊断、复跑验证和最终判断。
- 不要拆跑额外命令；除非这条命令本身无法执行，否则不要再调用第二次工具。
- 不要修改运行时代码、测试、依赖锁文件或测试脚本。
- 如果初次失败明确是 transient/flaky 信号，允许在同一次 shell 调用里复跑 `npm test` 验证。
...
function Show-TestSummary($path) {
  ...
  $matches = Select-String -Path $path -Pattern 'flaky-once|cache-warm|port-probe|transient|retry|temporary|warming|not ok|ok |# pass|# fail|node --test|tests? passed|Error' | Select-Object -First 24
...
$initialLog = Join-Path $env:TEMP 'mvp-flaky-transient-initial.log'
npm test *> $initialLog
...
$reran = $false
$flakyPattern = 'flaky-once: transient setup race|cache-warm: dependency cache still warming|port-probe: temporary port still busy'
if ($initialCode -ne 0 -and (Select-String -Path $initialLog -Pattern $flakyPattern -Quiet)) {
  $retryLog = Join-Path $env:TEMP 'mvp-flaky-transient-retry.log'
  npm test *> $retryLog
  $finalCode = $LASTEXITCODE
  $reran = $true
  Write-Output '--- retry npm test summary ---'
  Show-TestSummary $retryLog
}

Write-Output "initial_exit=$initialCode final_exit=$finalCode reran=$reran"
Write-Output '--- final git status ---'
git status --short
Write-Output '--- final tracked diff ---'
git diff --name-only
exit $finalCode
```
