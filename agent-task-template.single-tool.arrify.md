...
function Show-TestSummary($path) {
  ...
  $matches = Select-String -Path $path -Pattern 'converts iterable|Expected values|actual:|expected:|AssertionError|ERR_ASSERTION|not ok|ok |# pass|# fail|node --test' | Select-Object -First 28
...
$initialLog = Join-Path $env:TEMP 'mvp-arrify-initial.log'
npm test *> $initialLog
...
$changed = $false
if ($initialCode -ne 0 -and (Select-String -Path $initialLog -Pattern 'converts iterable|ERR_ASSERTION|AssertionError|Expected values' -Quiet)) {
  $source = Get-Content index.js -Raw
  $broken = "if (typeof value[Symbol.iterator] === 'function') {`n`t`treturn [value];`n`t}"
  $fixed = "if (typeof value[Symbol.iterator] === 'function') {`n`t`treturn [...value];`n`t}"
...
$finalLog = Join-Path $env:TEMP 'mvp-arrify-final.log'
npm test *> $finalLog
