...
function Show-TestSummary($path) {
  ...
  $matches = Select-String -Path $path -Pattern 'lint-quote-basic|not ok|ok |# pass|# fail|node --test|tests? passed|Error' | Select-Object -First 20
...
$initialLog = Join-Path $env:TEMP 'mvp-lint-quote-initial.log'
npm test *> $initialLog
...
$changed = $false
if ($initialCode -ne 0 -and (Select-String -Path $initialLog -Pattern 'use single quotes' -Quiet)) {
  $source = Get-Content index.js -Raw
  $fixed = $source -replace 'return "hello " \+ name;', "return 'hello ' + name;"
  if ($fixed -ne $source) {
...
$finalLog = Join-Path $env:TEMP 'mvp-lint-quote-final.log'
npm test *> $finalLog
