...
function Show-TestSummary($path) {
  ...
  $matches = Select-String -Path $path -Pattern 'escapes `-`|compatible with PCRE|not ok|×|√|# pass|# fail|test failed|test passed|xo && ava && tsd' | Select-Object -First 28
...
$initialLog = Join-Path $env:TEMP 'mvp-escape-string-regexp-initial.log'
npm test *> $initialLog
...
$changed = $false
if ($initialCode -ne 0 -and (Select-String -Path $initialLog -Pattern 'escapes `-`|compatible with PCRE|foo - bar' -Quiet)) {
...
Remove-Item -Recurse -Force .\node_modules\.cache\xo-linter -ErrorAction SilentlyContinue
$finalLog = Join-Path $env:TEMP 'mvp-escape-string-regexp-final.log'
npm test *> $finalLog
