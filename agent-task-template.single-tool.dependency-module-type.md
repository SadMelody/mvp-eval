...
- 不要运行 npm install；这个样本只验证 package module 配置。
...
function Show-TestSummary($path) {
  ...
  $matches = Select-String -Path $path -Pattern 'Cannot use import statement outside a module|Unexpected token|not ok|ok |# pass|# fail|node --test|tests? passed|Error|Warning' | Select-Object -First 24
...
$initialLog = Join-Path $env:TEMP 'mvp-dependency-module-type-initial.log'
npm test *> $initialLog
...
$changed = $false
if ($initialCode -ne 0 -and (Select-String -Path $initialLog -Pattern 'Cannot use import statement outside a module|Unexpected token' -Quiet)) {
  $package = Get-Content package.json -Raw | ConvertFrom-Json
  if ($null -eq $package.type) {
    $ordered = [ordered]@{}
    ...
    $ordered | ConvertTo-Json -Depth 8 | Set-Content -LiteralPath package.json -NoNewline
    $changed = $true
  } elseif ($package.type -eq 'commonjs') {
    $package.type = 'module'
    $package | ConvertTo-Json -Depth 8 | Set-Content -LiteralPath package.json -NoNewline
    $changed = $true
  }
}

$finalLog = Join-Path $env:TEMP 'mvp-dependency-module-type-final.log'
npm test *> $finalLog
