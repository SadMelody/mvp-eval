...
- 不要运行 npm install；这个样本只验证本地 dependency 配置。
...
function Show-TestSummary($path) {
  ...
  $matches = Select-String -Path $path -Pattern 'dependency config error|not ok|ok |# pass|# fail|node --test|tests? passed|Error' | Select-Object -First 24
...
$initialLog = Join-Path $env:TEMP 'mvp-dependency-basic-initial.log'
npm test *> $initialLog
...
$changed = $false
if ($initialCode -ne 0 -and (Select-String -Path $initialLog -Pattern 'dependency config error' -Quiet)) {
  $packageText = Get-Content package.json -Raw
  if ($packageText -match 'file:\./vendor/missing-runner' -and (Test-Path -LiteralPath 'vendor/mvp-local-runner/package.json')) {
    $packageText = $packageText -replace 'file:\./vendor/missing-runner', 'file:./vendor/mvp-local-runner'
    Set-Content -LiteralPath package.json -Value $packageText -NoNewline
    $changed = $true
  }
}

$finalLog = Join-Path $env:TEMP 'mvp-dependency-basic-final.log'
npm test *> $finalLog
