...
function Show-TestSummary($path) {
  ...
  $matches = Select-String -Path $path -Pattern 'flaky-once|transient|retry|not ok|ok |# pass|# fail|node --test|tests? passed|Error' | Select-Object -First 24
...
$initialLog = Join-Path $env:TEMP 'mvp-flaky-once-initial.log'
npm test *> $initialLog
...
$reran = $false
if ($initialCode -ne 0 -and (Select-String -Path $initialLog -Pattern 'flaky-once: transient setup race' -Quiet)) {
  $retryLog = Join-Path $env:TEMP 'mvp-flaky-once-retry.log'
  npm test *> $retryLog
