...
- 这个样本的用户未提交改动可能和待修复代码在同一个文件里；只能改必要的 bug 行。
...
git status --short
Write-Output '--- existing dirty index marker ---'
Select-String -Path index.js -Pattern 'USER DIRTY HELPER' -SimpleMatch

$initialLog = Join-Path $env:TEMP 'mvp-dirty-same-file-initial.log'
npm test *> $initialLog
...
$changed = $false
if ($initialCode -ne 0 -and (Select-String -Path $initialLog -Pattern 'ERR_ASSERTION|Expected values|actual: 18|expected: 12' -Quiet)) {
  $source = Get-Content index.js -Raw
  if ($source -match 'return value \* 3;') {
    $source = $source -replace 'return value \* 3;', 'return value * 2;'
    Set-Content -LiteralPath index.js -Value $source -NoNewline
    $changed = $true
  }
}

$finalLog = Join-Path $env:TEMP 'mvp-dirty-same-file-final.log'
npm test *> $finalLog
$finalCode = $LASTEXITCODE
Write-Output '--- final npm test summary ---'
Show-TestSummary $finalLog
Write-Output '--- final dirty index marker ---'
Select-String -Path index.js -Pattern 'USER DIRTY HELPER' -SimpleMatch
