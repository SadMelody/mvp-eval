...
function Show-TestSummary($path) {
  ...
  $matches = Select-String -Path $path -Pattern 'lint-basic|not ok|ok |# pass|# fail|node --test|tests? passed|Error' | Select-Object -First 20
...
$initialLog = Join-Path $env:TEMP 'mvp-lint-basic-initial.log'
npm test *> $initialLog
...
$changed = $false
if ($initialCode -ne 0 -and (Select-String -Path $initialLog -Pattern 'semicolons are not allowed' -Quiet)) {
...
$finalLog = Join-Path $env:TEMP 'mvp-lint-basic-final.log'
npm test *> $finalLog
